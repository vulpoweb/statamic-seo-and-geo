...
- Control panel views are styled by `resources/css/cp.css`, registered through the service provider's `$stylesheets` and published to `public/vendor/seo/css/`. Two things force that route: Tailwind classes used in an addon view are not in the control panel bundle (it is compiled from Statamic's own source), and a `<style>` block inside a CP Blade view is dropped by the control panel's Vue app. Run `php artisan vendor:publish --tag=seo --force` after updating the addon; `php please statamic:install` does it for you.
